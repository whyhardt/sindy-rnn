...
def main():
    ...
    config = {
        ...
        'num_sensors': 200,
        'latent_dim': 4,
        'polynomial_degree': 3,       # cubic dynamics (reference has z2*z3^2 etc.)
        'ensemble_size': 11,
        'window_size': 30,             # ~1 second of video at 30 FPS
        'train_fraction': 0.8,
        'dt': 1 / 30,                  # 30 FPS
        'encoder_type': 'gru',         # GRU for temporal context from sparse sensors
        'encoder_gru_hidden_dim': 512, # SHRED-like GRU hidden dimension
    ...
    model = SparseAutoencoderRNN(
        ...
        encoder_gru_hidden_dim=config['encoder_gru_hidden_dim'],
